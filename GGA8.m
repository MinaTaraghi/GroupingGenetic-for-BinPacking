...
%% Finding Biggest items
bigno=floor(popsize*0.3);
bigs=zeros(bigno,1);
ccitems=items;
for i=1:bigno
    [~,bigs(i)]=max(ccitems);
...
ccitems=ccitems(ccitems~=0);
itemno=itemno-bigno;
%% allocating items
for ch_index=1:popsize
iindex=2;
bindex=1;
chromosome{ch_index,bindex}=[bigs(1)];
for i=2:bigno
    chromosome{ch_index,bindex}=[bigs(2)];
    bindex=bindex+1;
end
rizindex=randperm(itemno+bigno);
rizindex=rizindex(~ismember(rizindex,bigs));
% unique(rizindex);
...
while(iindex<=itemno)
    ...
    if(summ+itemscc(iindex)<=binsize) % Current Bin can accomodate
        if(newBin)
            if(summ==0)
                chromosome{ch_index,bindex}=rizindex(iindex);
            else
                chromosome{ch_index,bindex}=[chromosome{ch_index,bindex},rizindex(iindex)];
            end
        else
            chromosome{ch_index,bindex}=[chromosome{ch_index,bindex},rizindex(iindex)];
        end
...
BinNo(ch_index)=max(bindex,bigno);
end
...
indices=randperm(popsize);
for i=1:invpopsize
    chromosome=invert(chromosome,indices(i),BinNo(indices(i)));
end
